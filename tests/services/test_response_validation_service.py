...
def validation(
    validation_type: str, value=None, error_message=None
) -> ResponseValidationVM:
    return ResponseValidationVM(
        type=validation_type, value=value, errorMessage=error_message
    )
...
def test_validate_response_returns_no_failures_for_no_content(
    validation_service,
) -> None:
...
def test_validate_response_returns_no_failures_without_validations(
    validation_service,
) -> None:
    assert validation_service.validate_response({}, build_response({"id": 1})) == []


def test_validate_response_returns_no_failures_when_all_validations_pass(
    validation_service,
) -> None:
    failures = validation_service.validate_response(
        {
...
def test_validate_response_reports_failed_validation(validation_service) -> None:
    ...
    assert (
        failures[0].message
        == "Validation failed for '$.id'. Expected: '42', but was: '41'."
    )
...
def test_validate_response_reports_field_that_should_not_exist(
    validation_service,
) -> None:
    failures = validation_service.validate_response(
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("empty", "  ", None, True),
        ("contains", "something", "thing", True),
        ("notcontains", ["a"], "b", True),
        ("containsall", ["a", "b"], ["b", "a"], True),
        ("containspropertywithvalue", [{"id": 2}], {"id": 2}, True),
    ],
)
def test_validate_supports_validation_types(
    validation_type: str, actual, expected, is_valid: bool, validation_service
) -> None:
    assert (
        validation_service._validate(actual, validation(validation_type, expected))
        is is_valid
    )
