...
def test_replace_dynamic_variables_replaces_today_with_default_format() -> None:
    result = DynamicVarsEvaluatorService()._resolve_dynamic_value("date={{DynamicToday}}")
...
def test_replace_dynamic_variables_generates_guid() -> None:
    result = DynamicVarsEvaluatorService()._resolve_dynamic_value("{{DynamicRandomGuid}}")
    assert re.fullmatch(
